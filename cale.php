    ...
    {    //输出日历
        ...
        for ($tmpb = 1; $tmpb <= date("t", mktime(0, 0, 0, $this->month, $this->day, $this->year)); $tmpb++) {    //输出所有日期
            if (strcmp($tmpb, $this->day) == 0) {  //获得当前日期，并采用特色颜色做为标记
                $flag = "background-color:#DD5269;width:30px;height:19px";

            } else {
                $flag = "background-color:#FAFDE2;width:30px;height:19px";
            }
            if ($tmpb == 1) {
                echo "<tr>";
            ...
            if (strcmp($this->getweek($this->year, $this->month, $tmpb), 0) == 0) {
                    echo "<tr><td align=center $flag ><button id='button$tmpb' style='$flag'>";
                    if ($tmpb >= 10)
                        echo "<a href='index.php?date=$this->year-$this->month-$tmpb'>$tmpb</a></button>";
                    else
                        echo "<a href='index.php?date=$this->year-$this->month-0$tmpb'>$tmpb</a></button>";
            } else {
                echo "<td align=center $flag><button id='button$tmpb' style='$flag'>";
                if ($tmpb >= 10)
                    echo "<a href='index.php?date=$this->year-$this->month-$tmpb'>$tmpb</a></button>";
                else
                    echo "<a href='index.php?date=$this->year-$this->month-0$tmpb'>$tmpb</a></button>";
